...
test.group('auth', (group) => {
  ...
  test('[store] - unable to authenticate with invalid credentials', async ({ assert, client }) => {
    const { response } = await client.post('/auth').json({
  ...
  test('[destroy] - able to delete token after logout', async ({ assert, client }) => {
    ...
    await client.delete('/auth').header('Authorization', `Bearer ${response.body.token}`)
